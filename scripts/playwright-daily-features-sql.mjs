...
const SQL = fs.readFileSync(path.join(process.cwd(), 'supabase', 'sql', 'daily-features.sql'), 'utf8')
...
try {
  ...
  await runSql(page)
  await shot(page, 'daily-features-sql')
  const body = await page.locator('body').innerText()
  console.log(body.replace(/\s+/g, ' ').slice(0, 1400))
  console.log(/chat_messages|scheduled_events|Success/i.test(body) ? 'SQL_OK' : 'SQL_CHECK')
} catch (error) {
  await shot(page, 'daily-features-sql-error')
